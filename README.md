...
<p align="center">
  支持多Agent协作 • Durable Execution • 知识库增强 • MCP服务器集成 • 工具治理 • 实时对话
</p>
...
<p align="center">
  ...
  <a href="docs/technical_report_2026-03.md">🧾 技术报告</a> •
...
### 1. 主Agent支持 Durable Execution / Checkpoint / Resume
主执行链已基于 LangGraph 落地可恢复执行：
- 为每次运行生成 `run_id`
- 使用 MySQL 持久化 LangGraph checkpoint
- 支持 `resume=true` 继续执行中断任务
- 高风险工具可在审批后恢复，而不是整次任务重跑

### 2. 结构化输出升级为 Structured Outputs
项目已在关键链路收敛到更稳定的结构化输出模式：
- Query Rewrite 使用结构化返回而非 JSON 修补
- LingSeek 任务规划使用 Pydantic schema 约束输出
- StructuredResponseAgent 统一基于 `with_structured_output(..., method="function_calling")`

### 3. 新增 Tracing / Observability / Eval 基础能力
系统新增运行级观测链路：
- `agent_run` 记录一次完整执行
- `agent_span` 记录模型/工具调用 span
- `tool_execution_audit` 记录工具审批与审计
- `agent_eval_record` 为后续评测与回归分析提供数据基础

### 4. RAG 从检索拼接升级为 Context Engineering
知识增强链路不再只是“检索 + rerank + 文本拼接”：
- 引入 Query Rewrite
- 输出 `KnowledgeContextPackage`
- 注入精简上下文、引用信息和 retrieval strategy
- 同时支持主链自动注入和知识工具显式调用
...
这使项目从“能调用工具”升级为“可治理、可审计、可审批的 Agent 工具体系”。
...
AgentChat 是一个面向生产场景的智能体平台，基于大语言模型构建，提供多 Agent 协作、知识增强、工具调用、MCP 集成和流式对话能力。系统采用前后端分离架构，并在当前版本中补齐了主执行链的 durable execution、Structured Outputs、工具治理与运行观测能力。

### 🌟 核心亮点

- 🤖 **多模型支持**: 集成OpenAI、DeepSeek、Qwen等主流大语言模型
- 🧠 **智能Agent**: 支持多Agent协作，具备推理和决策能力
- 💾 **Durable Execution**: 基于 LangGraph + MySQL Checkpoint 的可恢复执行
- 🧱 **Structured Outputs**: 关键规划链路使用 schema 约束输出，减少 JSON 脆弱性
- 📚 **知识库增强**: RAG + Context Engineering 实现精准知识检索和上下文注入
- 🔧 **工具生态**: 内置多种实用工具，支持自定义扩展
- 🌐 **MCP集成**: 支持 Model Context Protocol 服务器与标准化能力接入
- 🔍 **可观测性**: 提供 run/span/tool audit/eval 的基础观测能力
- 💬 **实时对话**: 流式响应，提供流畅的对话体验
...
- [AgentChat 技术报告（2026-03）](docs/technical_report_2026-03.md)

报告覆盖当前代码基线下的：
- 总体架构与模块分层
- LangGraph 主执行链与持久恢复机制
- Structured Outputs、MCP 集成、RAG context engineering
- tracing / observability / eval 现状
- 当前风险与下一阶段演进路线
...
- 🔧 **任务自动化**: 智能分解复杂任务，自动执行
- 💾 **可恢复执行**: 基于 LangGraph Checkpoint 的暂停、恢复和续跑
- 🛡️ **工具审批**: 高风险工具支持 human-in-the-loop 审批后继续执行
...
- 🧩 **智能分块**: 语义级别的文档分割和处理
- 🔍 **混合检索**: 向量检索、搜索引擎检索与重排结合
- 💡 **Context Engineering**: 输出结构化 context package，而不是简单文档拼接
- 🧾 **引用信息**: 返回 compact context、citation 和 retrieval strategy
- 🗂️ **知识组织**: 分类管理，标签系统
- 📈 **使用统计**: 知识库使用情况分析
...
- 🔄 **前后端分离**: Vue3 + FastAPI
- 📡 **实时通信**: SSE 流式输出与实时事件回传
- 🕸️ **Agent图执行**: 基于 LangGraph 构建主执行链
- 💾 **多数据库**: MySQL、Redis、ChromaDB
- 🐳 **容器化**: Docker部署，易于扩展
- 📈 **可观测**: run/span/tool audit/eval 的基础追踪体系
...
| **可恢复执行** | 中断后基于 checkpoint 继续执行 | LangGraph + MySQL |
| **结构化输出** | 降低 JSON 解析脆弱性 | Pydantic Schema + Function Calling |
| **运行追踪** | 记录 run、span、tool audit、eval | FastAPI + SQLModel |
...
- [技术报告 2026-03](docs/technical_report_2026-03.md) - 当前架构、主链能力与演进路线
